local function trucate_left(text, max_length)
...
local function config_lualine(colors)
	...
	local filename = {
		function()
			...
			if file == "" then
				return "[No Name]"
			end -- Handle unsaved buffers

			local parent_dir = vim.fn.fnamemodify(file, ":h:t") -- Get parent directory name
			local name = vim.fn.fnamemodify(file, ":t") -- Get file name

			if parent_dir == "." then -- If there's no parent, just return the filename
				return name
			end

			local text = parent_dir .. "/" .. name
			return trucate_left(text, 50)
		end,
